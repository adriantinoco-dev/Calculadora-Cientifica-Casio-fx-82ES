    ...
    def limpar_expr(expr: str) -> str:
        # 1. Notação Casio: ×10 só quando seguido de superescritos reais (⁰¹²³⁴⁵⁶⁷⁸⁹⁻)
        #    Isso evita confundir ×100, ×1000 etc. com notação científica
        expr = re.sub(r"×10([⁰¹²³⁴⁵⁶⁷⁸⁹⁻]+)",
                    lambda m: "e" + m.group(1).translate(_RSUP), expr)
        # 2. x² — seguro agora que expoentes científicos (ex: 10⁻²) já foram consumidos
        expr = expr.replace("²", "**2")
        # 3. Superescritos remanescentes e normalização PT-BR
        expr = expr.translate(_RSUP)
    ...
    def para_python(expr: str) -> str:
        ...
        # % → /100 (após limpar_expr, que agora não confunde ×100 com notação científica)
    ...
    class Calculadora:
        ...
        def inserir_percentual(self):
            """Insere % no display; para_python converte para /100 na hora de calcular."""
            ultimo = self.expr[-1] if self.expr and not self.mostrando else ""
            if ultimo.isdigit() or ultimo == ")":
                self.expr += "%×"
                self._atualizar()
            # Se não há número antes, não faz nada
        ...
        def _tecla(self, ev):
            ...
            elif c == "%":            self.inserir_percentual()
